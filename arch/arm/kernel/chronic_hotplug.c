 ...
 * Modified by Matt Filetto <matt.filetto@gmail.com> for Snapdragon 801 SoCs
 *
...
ssize_t show_enable_core_thresholds(struct kobject *kobj,
...
ssize_t store_enable_core_thresholds(struct kobject *kobj,
...
static void hotplug_decision_work_fn(struct work_struct *work)
{
	...
	if (likely(!(flags & HOTPLUG_DISABLED))) {
		...
		if (unlikely((avg_running >= ENABLE_ALL_LOAD_THRESHOLD) && (online_cpus < available_cpus))) {
			pr_info("chronic_hotplug: Onlining all CPUs, avg running: %d\n", avg_running);
			/*
...
#if DEBUG
			pr_info("chronic_hotplug: Onlining single CPU, avg running: %d\n", avg_running);
#endif
			if (delayed_work_pending(&aphotplug_offline_work))
				cancel_delayed_work(&aphotplug_offline_work);
			...
			if (!(delayed_work_pending(&aphotplug_offline_work))) {
#if DEBUG
				pr_info("chronic_hotplug: Offlining CPU, avg running: %d\n", avg_running);
#endif
...
#if DEBUG
				pr_info("chronic_hotplug: Clearing boostpulse flags\n");
#endif
			}
		}
	}
...
bool apget_enable_chronic_hotplug(void)
{
	return isEnabled;
}

void apenable_chronic_hotplug(bool state)
{
...
static void __cpuinit hotplug_online_all_work_fn(struct work_struct *work)
{
	...
	for_each_possible_cpu(cpu) {
		if (likely(!cpu_online(cpu)) && (cpu)) {
			cpu_up(cpu);
#if DEBUG
			pr_info("chronic_hotplug: CPU%d up.\n", cpu);
#endif
		}
	}
	/*
...
static void __cpuinit hotplug_offline_all_work_fn(struct work_struct *work)
{
	...
	for_each_possible_cpu(cpu) {
		if (likely(cpu_online(cpu) && (cpu))) {
			cpu_down(cpu);
#if DEBUG
			pr_info("chronic_hotplug: CPU%d down.\n", cpu);
#endif
		}
	}
}
...
{
#if DEBUG
	pr_info("chronic_hotplug: Clearing pause flag\n");
#endif
	flags &= ~HOTPLUG_PAUSED;
}
...
void hotplug_disable(bool flag)
{
	if (flags & HOTPLUG_DISABLED && !flag) {
		...
		flags &= ~HOTPLUG_PAUSED;
#if DEBUG
		pr_info("chronic_hotplug: Clearing disable flag\n");
#endif
		schedule_delayed_work_on(0, &hotplug_decision_work, 0);
	} else if (flag && (!(flags & HOTPLUG_DISABLED))) {
		flags |= HOTPLUG_DISABLED;
#if DEBUG
		pr_info("chronic_hotplug: Setting disable flag\n");
#endif
...
#if DEBUG
			pr_info("chronic_hotplug: %s: %d CPUs online\n", __func__, num_online_cpus());
#endif
			if (delayed_work_pending(&aphotplug_offline_work)) {
#if DEBUG
				pr_info("chronic_hotplug: %s: Cancelling aphotplug_offline_work\n", __func__);
#endif
				cancel_delayed_work(&aphotplug_offline_work);
				flags |= HOTPLUG_PAUSED;
				schedule_delayed_work(&hotplug_unpause_work, HZ );
...
static void chronic_hotplug_early_suspend(struct early_suspend *handler)
{
	if (!isEnabled)
		return;

	pr_info("chronic_hotplug: early suspend handler\n");
	...
	if (num_online_cpus() > 1) {
		pr_info("chronic_hotplug: Offlining CPUs for early suspend\n");
		schedule_work_on(0, &hotplug_offline_all_work);
	}
}

static void chronic_hotplug_late_resume(struct early_suspend *handler)
{
	if (!isEnabled)
		return;

	pr_info("chronic_hotplug: late resume handler\n");
	flags &= ~EARLYSUSPEND_ACTIVE;

	schedule_delayed_work_on(0, &hotplug_decision_work, HZ);
}

static struct early_suspend chronic_hotplug_suspend = {
	.suspend = chronic_hotplug_early_suspend,
	.resume = chronic_hotplug_late_resume,
};
#endif /* CONFIG_HAS_EARLYSUSPEND */

static struct global_attr enable_core_thresholds_attr = __ATTR(enable_core_thresholds, 0666, show_enable_core_thresholds, store_enable_core_thresholds);

static struct attribute *chronic_hotplug_attributes[] = {
	&enable_core_thresholds_attr.attr,
	NULL,
};

static struct attribute_group chronic_hotplug_attr_group = {
	.attrs = chronic_hotplug_attributes,
	.name = "chronic_hotplug",
};

static int __init chronic_hotplug_init(void)
{
	int rc;
	rc = sysfs_create_group(cpufreq_global_kobject,
				&chronic_hotplug_attr_group);

	pr_info("chronic_hotplug: v0.01 by _thalamus modded by Matt Filetto for Snapdragon 801 SoC\n");
	pr_info("chronic_hotplug: %d CPUs detected\n", CPUS_AVAILABLE);
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	register_early_suspend(&chronic_hotplug_suspend);
#endif
	return 0;
}
late_initcall(chronic_hotplug_init);
